#!/usr/bin/env bash
# close.sh <slug> <shipped|abandoned|dead-end> "<reason>" [--delegated]
...
usage() { echo "usage: close.sh <slug> <shipped|abandoned|dead-end> \"<reason>\" [--delegated]"; exit 1; }
delegated=""
if [ $# -eq 4 ] && [ "$4" = "--delegated" ]; then delegated=1; set -- "$1" "$2" "$3"; fi
[ $# -eq 3 ] || usage
slug="$1"; state="$2"; reason="$3"
case "$state" in (shipped|abandoned|dead-end) ;; (*) echo "FAIL: state must be shipped|abandoned|dead-end"; usage;; esac
dir=".sdlc/work/$slug"
